...
export class LandingComponent implements OnInit {
  ...
  onWindowScroll() {
    if (document.documentElement.scrollTop > 80) {
      document.getElementsByClassName('navbar')[0].classList.add('nav__color__change');
    }
    else {
      document.getElementsByClassName('navbar')[0].classList.remove('nav__color__change');
    }
  }


  private initBlobFigure() {
    new TimelineMax({
      yoyo: !0,
      repeat: -1
    }).to(".blob", 3, {
